...
def test_a_compatible_directory_whose_every_path_verifies_is_labelled_verify_not_resume(tmp_path):
    ...
    """
    from md_tools.ais.run import RUN_IDENTITY_VERSION, decide_run_disposition, run_identity_document

    out = tmp_path / "AIS"
    document = run_identity_document(
        fingerprint="f" * 64, topology_facts={"sha256": "t" * 64},
        system_facts={"sha256": "s" * 64}, source_facts={"sha256": "x" * 64},
        source_format="dcd", schedule={"switching_steps": 10},
        ais={"tau_start": 0.5, "tau_end": 0.0, "number_of_paths": 0},
        dynamics={"seed": 1}, chosen=[], reporting={}, resolved_config=None)
...
def test_an_incomplete_compatible_directory_without_resume_is_refused_before_any_byte_changes(
    ...
    """
    from md_tools.ais.run import decide_run_disposition, run_identity_document
    ...
    document = run_identity_document(
        ...
        dynamics={"seed": 1}, chosen=[7], reporting={}, resolved_config=None)
    _write_identity(out, document)
...
def test_an_incompatible_directory_is_refused_by_naming_what_differs_even_with_resume(tmp_path):
    ...
    new = run_identity_document(
        fingerprint="f" * 64, topology_facts={"sha256": "t" * 64},
        system_facts={"sha256": "s" * 64}, source_facts={"sha256": "x" * 64},
        source_format="dcd", schedule={"switching_steps": 10},
        ais={"tau_start": 0.5, "tau_end": 0.0, "number_of_paths": 1},
        dynamics={"seed": 99}, chosen=[7], reporting={}, resolved_config=None)
