...
description: Inspect a Linux workstation and safely install or validate Amber26, AmberTools26, and OpenMM 8.5.2. Use when a user asks to prepare an MD machine, install Amber/OpenMM, inspect CPU/RAM/disk/NVIDIA/CUDA resources, choose an installation prefix, validate user-supplied Amber or pmemd archives, prefer CUDA over OpenCL, or diagnose an incomplete MD software installation.
---
...
2. Summarize CPU, RAM, free disk, OS, compilers, NVIDIA GPUs, driver, `nvcc`, and package managers.
3. Ask for:
...
An NVIDIA driver can be sufficient for a packaged OpenMM CUDA runtime, but building `pmemd.cuda` also requires a compatible CUDA toolkit and compiler. Report these capabilities separately. Never install or replace a system GPU driver without explicit user and administrator approval.
...
Configure a separate build directory and the user-selected installation prefix. Enable CUDA only when the detected toolkit is supported by the bundled Amber26 build system. Build with a conservative parallel job count based on RAM and CPU. Capture configure, build, install, and test logs.
...
Run the release-provided Amber tests appropriate to the installed components. Confirm at least `tleap`, `sander`, `pmemd`, and `cpptraj`; require `pmemd.cuda` when CUDA was selected. Then run `scripts/verify_install.py` with the exact Amber prefix and OpenMM Python.
...
- CUDA/CPU/OpenCL status for each engine;
