...
<PREFIX>/
  ...
  build/amber26/        # out-of-source Amber build
...
| --- | --- | --- |
| NVIDIA GPU and working driver; compatible toolkit present | CUDA | Build `pmemd.cuda` after checking Amber26's bundled compatibility notes |
...
and source-built Amber have different CUDA prerequisites.
...
5. Use an out-of-source build and set the install prefix to `<PREFIX>/amber26`.
6. Enable CUDA only when Amber26 documents support for the detected toolkit and compiler. Preserve
   the complete CMake configure output.
7. Choose parallel jobs conservatively. Reduce jobs on low-memory machines rather than allowing
   the compiler to exhaust RAM.
8. Run release-provided tests and capture their logs before declaring success.
...
#!/usr/bin/env bash
set -e
export AMBERHOME=<PREFIX>/amber26
...
python scripts/verify_install.py \
  --amberhome <PREFIX>/amber26 \
  --python <PREFIX>/envs/openmm-8.5.2/bin/python \
  --require-cuda \
  --output <PREFIX>/manifests/validation.json
```

Omit `--require-cuda` only for an explicitly selected CPU/OpenCL installation. In addition to this
probe, run the Amber release test targets and preserve their logs.
...
- compiler, CMake, driver, and CUDA toolkit versions used;
