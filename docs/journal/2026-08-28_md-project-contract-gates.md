...
Two things found while considering it, one fixed here and one not:

* `net_charge_e`, `formal_charge` and `n_atoms` were dropped from `forcefield.json` by the same
  function that dropped `charge_scheme`. **Fixed**, because a cache is only safe if two builds of
  the same input produce records that can be compared, and these are what makes that possible.
* **the ETKDG embedding seed is not persisted anywhere in a generated dataset.** `structure` and
  `run` are both null in `resolved_sys.config.yaml`, and no other record carries it. AM1-BCC
  charges are conformer-dependent, and this repository already refuses a null seed on the grounds
  that a random embedding would make a prepared system irreproducible -- so the seed is exactly the
  kind of thing the record exists to hold. **Not fixed here**: it is the structure-preparation
  record rather than the charge path, and widening this change to reach it would blur a focused
  review.

The consequence for the eventual cache is that its key is (canonical isomeric SMILES, embedding
seed, charge scheme) -- not SMILES and scheme, as the upstream report proposed. Without the seed
in the record a cache hit could not be shown to be legitimate.
