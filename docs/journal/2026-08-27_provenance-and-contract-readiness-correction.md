...
`provenance_min.template_identity()` is now the single resolution of "which MD-tools is this".
...
```text
sys-gen: the MD-tools generating this dataset is a Git checkout at 5d5fb472ea82 with
...
Mismatching (each fails with *"not generated by the same MD-tools"*): `dataset.yaml`,
