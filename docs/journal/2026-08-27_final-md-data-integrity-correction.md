...
running MD-tools actually is, from a Git checkout (`git rev-parse HEAD`) or PEP 610
...
```text
dataset.templates.commit is 'bbbb...', but the MD-tools actually generating this dataset
...
```text
this MD-tools cannot establish its own exact commit, so the claim cannot be verified.
...
were not generated by the same MD-tools`.
...
* **Preflight compares generator records; it cannot re-establish them.** A generated project
  deliberately cannot import `md_tools`, so if every record was written by the same wrong
