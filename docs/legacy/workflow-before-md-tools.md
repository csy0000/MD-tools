> **Historical.** This describes the workflow as it was before the 2026-09-01 MD-tools
> migration, when the package was called MD-templates and its commands were `md-openmm setup`,
> `sys-config`, `sys-gen`, `md-gen`, the `openmm-md` executor and the `md-template` installer.
> None of those exist any more. It is kept because datasets registered before that date were
> produced by exactly this workflow, and their provenance is not rewritten.
>
> For the current interface see the README at the root of this repository.
