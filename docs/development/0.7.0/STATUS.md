...
| A1 | `combine-topology` construction | S2 | IMPLEMENTED (callable layer; CLI not wired). Complex-leg fixture (capped ALA + ethane, ff14SB/TIP3P) recovers both endpoints; barostat carried through. CMAP row NOT RUN: `build-top kind: complex` with ff19SB+OPC refuses every ligand package on a 1-4 scale comparison against OPC's rounded `0.833333` — a 0.6.0 ligand-module issue, reported to MD-tools-0.6.0 | integrated at `6df0df6`; [handoffs/S2.md](handoffs/S2.md); 114 passed on Reference, `MD_DATA` at an empty temp root |
