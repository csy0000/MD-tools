...
| W7 | OpenMM harmonic model, 5 windows × 100 000 steps | MBAR, BAR, EXP, TI ΔG | closed form | gate | CPU `--cpu` | `pytest tests/test_alchemy_windows.py -m slow` | S4.md §Evidence | see S4.md |
...
| N1 | NPT windows on a periodic System | pV in u; barostat present only in the sampling Context | a periodic analytic model | as W1–W4 | CPU, then CUDA | — | — | NOT RUN: the S4 harmonic OpenMM model has no box. The code path exists; it is untested |
