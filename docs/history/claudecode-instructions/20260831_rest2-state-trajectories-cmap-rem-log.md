...
The wildcard is for discovery only. Never pass `$DATA_ROOT/MD_DATA*` or another unresolved glob to `rm`, `mv`, `find -delete`, or any recursive destructive command.
...
```text
$DATA_ROOT/MD_DATA/validation/2026-08/alanine-dipeptide-nvt-rest2/
├── inputs/
├── common/
├── REST2/
└── REST2_ext1/
