...
Implementation: `src/escort_ais/systems/explicit_baseline.py`.
...
```bash
conda activate escort-ais-explicit  # REQUIRED -- see "AmberTools must be on PATH" below
...
are installed in the `escort-ais` environment, and AM1BCC then silently becomes unavailable.
`build_forcefield` checks the registry and **fails loudly** rather than letting the charges fall back
to a different method under an AM1BCC label. Always `conda activate escort-ais-explicit`.
...
own: conditional BAR draws its forward AIS seeds per generation from ~1 ns slices of the hot chain,
...
(`escort_ais.systems.explicit_baseline.DEFAULTS`, dumped in full as `scripts/config_defaults.json`).
