...
| `umbrella.file` | none | torsion restraints, the SAME on every rung, as in [REST2](../REST2/README.md#restraining-every-rung). Needs `collective_variables.file`, and is **refused together with `reservoir.enabled`**: a reservoir sample is drawn from a distribution generated WITHOUT the bias, so refreshing the top rung from one would install an unrestrained configuration into a restrained ladder, which samples neither ensemble. Generate the reservoir under the same restraints — a different file — or drop one of the two |
