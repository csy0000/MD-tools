...
difference stays the definition so that a non-linear schedule, when one is added, cannot change
what `work` means.
...
* no non-linear schedule (Amber's smoothstep `S_p(λ)`), no partial λ window;
* no atom mapping, softcore or dummy atoms;
* it cannot generate its own source ensemble. AIS switches *away from* an equilibrium ensemble and
