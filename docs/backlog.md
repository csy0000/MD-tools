...
| `$MD_DATA` isolation is enforced by an autouse fixture and a frozenset, and nothing FAILS when it is lost | S0 and the MD-tools-0.6.0 session, 2026-09-22, while gating 0.6.1. `tests/conftest.py` isolates every test to an empty `MD_DATA`, and `USES_THE_MACHINES_MD_DATA` is empty -- but a name added to that frozenset, a constant imported from elsewhere, or a conftest deeper in `tests/` shadowing the fixture would all restore the 0.6.0 defect silently, where registering a package turned two ligand tests red with a message naming neither the catalog nor the machine. The gate session's check is a per-lane read of the EFFECTIVE state, which is right and is not in the suite | open: a canary TEST rather than a reviewer's habit -- assert inside a running test that `os.environ["MD_DATA"]` is a fresh empty directory and is NOT `default_catalog_root()`'s machine root, and show it FAILS when the fixture is disabled. A rule enforced by a frozenset nobody re-reads is a rule until the day it is not |
| **"registered" reads as "complete", and it does not mean that**: a dataset registers successfully while lacking the inputs its own build record cites | MD-tools-0.6.0, 2026-09-22, verified in `$MD_DATA/2026/tutorials/tyk2-ejm31-rest2-ligand-pocket-8x5ns`: `build/built.log` names `input/complex.pdb` (lines 6, 91, 119) and `ligands/LOCAL-DKNAYSZNMZIMIZ/param_bd1388e5fe3e` (line 1447) and NEITHER is in the dataset. `export-reference` verifies both by digest, so both registered tutorial datasets are **permanently un-exportable** -- registration is write-once, so it is not repairable in place. Nothing at registration time said so | open, 0.6.2: a manifest-COMPLETENESS check -- registration parses the build record and refuses, or at minimum warns by name, when a cited input is absent from the tree being registered. Until then the caveat is documented. Related and separate: `export-reference` cannot read a `build-md`-produced ladder at all, because `reference/export.py:196` and `reference/rest2_export.py:374` both look for `<run>/<stage>.log` while `build/md.py:1907` writes the records under `remd_records/` -- BOTH entry points need the fix, and the test must exercise a ladder produced by `run.sh` rather than a fixture |
