...
| a placement record names CUDA DEVICE INDICES, and CUDA's default order is not nvidia-smi's, so a record saying "devices 0-3" does not identify which cards ran | S1, 2026-09-22, measured: it started MPS with `CUDA_VISIBLE_DEVICES=1,2,3,4` on four granted cards and the server's contexts appeared on nvidia-smi cards **2,3,4,5** -- one card nobody had granted -- with granted card 1 idle. CUDA's default `CUDA_DEVICE_ORDER` is FASTEST_FIRST, and this host is one RTX A5000 (nvidia-smi 0) among eight RTX 3080s, so the two numberings differ by one. With `CUDA_DEVICE_ORDER=PCI_BUS_ID` a one-particle Context on index 0 lands on the A5000 and index 1 on a 3080, matching nvidia-smi; without it the same call fails with CUDA_ERROR_INVALID_DEVICE. The 2026-09-21 TYK2 campaign therefore probably ran on 2,3,4,5 rather than the granted 1,2,3,4 -- no measured number is affected (all eight 3080s are identical and the A5000 was never in the set) but "the four granted cards" was an inference, not an observation | open: (1) every session exports `CUDA_DEVICE_ORDER=PCI_BUS_ID` for daemons AND clients, so a granted card number means one thing; (2) the placement record should carry each rank's **PCI bus id**, which is unambiguous, beside the device index it used -- a record that cannot say which physical card ran cannot support a claim about homogeneity, contention or a grant being honoured |
| an estimator whose weights have been destroyed reports a WEIGHTED estimate equal to its unweighted one, and nothing fails | hpREST2, 2026-09-22, in ITS OWN SMC layer, not `md_tools` -- verified here: `md_tools/ais/` has no population, no weight vector and no resampling (`two_state` is a single-path sampler reading `dV/dlambda = V1 - V0` at frozen coordinates) and `alchemy/estimators.py` does not resample. The shape is general and worth guarding anyway: its `if resample:` fired after EVERY leg including the last, and a resample sets `log_w = 0`, so the final population was equally weighted and a "weighted estimate" was a post-resample fraction. It is unbiased in expectation -- which is why a validation against a known answer PASSES -- but resolution is destroyed: every probability quantises to 1/N, and a leaf at 0.43 of one walker's weight in 1000 was reported as 0.000%. ESS read 1000/1000 because the weights had just been reset | open: (1) **a resample belongs BETWEEN legs, never after the last one**, since after the final leg there is no next proposal to concentrate and resampling replaces a weighted average with one draw from it; (2) the cheap detector, worth asserting wherever we weight anything: **if the weighted and unweighted estimates agree to the last digit, the weights are gone** -- every row reading 18.800%/18.800% is a symptom, not a coincidence; (3) assert `log_w` is not identically zero at the point of estimation |
