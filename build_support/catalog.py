...
#: Generated by the in-tree backend immediately before packaging, so it is build OUTPUT that
#: happens to land inside `src/`. Including it would make the source digest depend on the build
#: that is computing it: a wheel built from an unmodified sdist would see a tree that no longer
#: matched the sdist's recorded digest and would refuse to inherit its commit.
EXCLUDED_EXACT_PATHS = frozenset({"src/md_templates/_build_info.py"})


def _is_excluded(relative: Path, provenance_filename: str) -> bool:
    parts = relative.parts
    if relative.as_posix() in EXCLUDED_EXACT_PATHS:
        return True
