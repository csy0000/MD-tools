...
def bundlev2_module():
    """The bundle contract as this engine sees it: the core file format plus its own extras.

    Phase 3 split the contract -- `core.bundle` owns normalised paths, the checksum domain and the
    environment record, while `bundleinfo` owns the counts and force-field provenance that need a
    built System. Callers here want both under one name, and the legacy `bundlev2` name meant
    exactly that union, so it is assembled in one place rather than at every call site.
    """
    from types import SimpleNamespace

    from ...core import bundle as contract
    from . import bundleinfo

    return SimpleNamespace(
        **{name: getattr(contract, name) for name in dir(contract) if not name.startswith("_")},
        topology_counts=bundleinfo.topology_counts,
        forcefield_provenance=bundleinfo.forcefield_provenance,
    )
