...
# Defined once in `md_templates.core.hashing` and re-exported here, because the fingerprint
# projection and this reader must agree on bytes exactly. Two implementations that drifted by a
# separator or a sort order would hash one document two ways, and the first symptom would be a
# bundle refusing its own configuration.
from ..core.hashing import canonical_json, sha256_file, sha256_text  # noqa: E402,F401
