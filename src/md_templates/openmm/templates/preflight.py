...
def check_dataset(here, project, config):
    ...
    results += check_template_provenance(here, project, manifest=document)
    return results


def check_template_provenance(here, project, *, manifest=None, stage=None):
    """The generator commit agrees across every record that names it.

    `dataset.yaml` says which MD-templates the dataset was generated by, and each `stage.yaml`
    says which one wrote that stage's scripts. If a dataset's system was built from one checkout
    and its run scripts from another, the single recorded provenance is true of only half of the
    tree -- and it is the half nobody checks that misleads.

    This compares RECORDS. Establishing what the generator actually was is `md_data_contract`'s
    job at generation time; nothing here can rerun that, and a generated project deliberately
    cannot import `md_templates`.
    """
    if manifest is None:
        path = Path(project) / "dataset.yaml"
        if not path.is_file():
            return []
        manifest = _read_yaml(path) or {}
    declared = ((manifest.get("templates") or {}).get("commit") or "").strip()
    if not declared:
        return [_no("template provenance", "dataset.yaml names no templates.commit")]

    seen = {}
    if stage is None:
        stage = _read_yaml(Path(here) / "stage.yaml") or {}
    recorded = (stage.get("template_commit") or "").strip()
    if recorded:
        seen["this stage's stage.yaml"] = recorded
    written = _read_yaml(Path(project) / "provenance.yaml") or {}
    generated = (written.get("implementation") or {}).get("git_commit")
    if generated:
        seen["provenance.yaml"] = str(generated).strip()

    disagreeing = {where: value for where, value in seen.items()
                   if value.lower() != declared.lower()}
    if disagreeing:
        detail = "; ".join(f"{where} says {value[:12]}" for where, value in
                           sorted(disagreeing.items()))
    ...
    if not seen:
        return [_skip("template provenance",
                      f"dataset.yaml records {declared[:12]}; nothing else in this project names "
                      f"a generator commit to check it against")]
    return [_ok("template provenance",
                f"{declared[:12]} agrees across {len(seen) + 1} record(s)")]
