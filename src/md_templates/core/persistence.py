...
#: `save_restart`/`load_restart` moved to `md_templates.openmm.restart` in Phase 3: they take a
#: running `Simulation` and serialize OpenMM objects, so they belong with the provider. They are
#: re-exported through the legacy `md_templates.openmm.runstate` path by that module, keeping the
#: historical API intact while core stays importable with no engine installed.
