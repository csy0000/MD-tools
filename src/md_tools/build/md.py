...
One shape: `min.py`, the equilibration chain, then the production script, plus `run.sh`. There was
once a second -- `--all-in-one`, a single `md.py` running every stage in one process -- and it was
retired in 0.5.4. It ran the same stages under the same resolved settings, so it bought nothing a
reader could see, while every feature had to be built twice and the split form was the one that
got tested. What it did carry alone was a whole-chain preflight, and that moved HERE: generation
validates every stage of the chain before it writes the first script.
"""
...
def _run_sh(plan: list[dict[str, Any]], *, protocol: str,
    ...
    lines = ['#!/usr/bin/env bash',
             ...
             'TOPOLOGY="${HERE}/../build/built.pdb"',
             'SYSTEM="${HERE}/../build/built.xml"',
             'SOURCE=""',
             '# Leading arguments that are not flags name the paths, in order, and are consumed so',
             '# that a path this script has already used is not forwarded on as though it were a',
             '# flag. A flag ends them: `./run.sh --cpu` is the default paths plus --cpu, and',
             '# taking "--cpu" as the topology refused every run started that way.',
             ...
             'if [[ $# -ge 1 && "${1}" != -* ]]; then TOPOLOGY="${1}"; shift; fi',
             'if [[ $# -ge 1 && "${1}" != -* ]]; then SYSTEM="${1}"; shift; fi',
             'if [[ $# -ge 1 && "${1}" != -* ]]; then SOURCE="${1}"; shift; fi',
             'cd "${HERE}"',
             '',
             'if [[ ! -f "${TOPOLOGY}" ]]; then',
...
in the installed md_tools package, and the ladder is in `resolved.config` beside this file.
    ...
    mpiexec -n {states} python {protocol}.py -ng {states} -p ../build/built.pdb --groupfile remd_groupfile.1

Each line of the group file names its state's -s, a saved state from
`md-openmm build-top --rest2-scaler`; -s on the command line is refused.
...
def build_scripts(*, config_path: Path | None, out_dir: Path,
    ...
    #
    # EVERY PROTOCOL, not only a ladder. Generation validates the whole chain below, and a chain
    # cannot be validated against a System that does not exist: the masses behind a timestep, the
    # box behind an ensemble and the forces the scaling convention has to place are all read from
    ...
    #
    # This is the guarantee `--all-in-one` carried alone, moved to the surface that generates the
    # chain rather than to one of the two shapes that ran it. A split run preflights one stage as
    # it starts, so a last stage asking 4 fs of unrepartitioned masses, or NPT of an implicit
    # System, was found only after every earlier stage had run -- and by then the queue time is
    # spent and the run directory holds a chain that cannot be finished.
    #
    # The plan and its paths are computed HERE, above the mkdir, because this pass validates the
    # very paths the writing pass below produces.
    #
    # AND COMPUTED AGAIN below, which is deliberate rather than an oversight: `resolved` gains the
    # `file=` provenance of the copied-in cv and umbrella definitions between here and there, and
    # the writing pass must plan from the document it actually writes. The two differ only by that
    ...
    run_sh.write_text(_run_sh(plan, protocol=protocol,
    ...
    for directory, document, why, shared in _script_directories(plan, run=run, dataset=dataset,
        ...
        # CREATED FIRST, because a directory may hold a shared input before it holds a script:
        # `min/` and `input/` are siblings of the run and are written here whether or not the
        # per-stage loop above has reached them. The write once failed with a bare
