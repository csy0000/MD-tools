...
with no `--solvent` writes `amber14-all.xml` + `amber14/tip3p.xml`. ff19SB + OPC remains available
unchanged as `--solvent OPC`; the protein force field and the water model are one selection, not two
independent keys, because each pair is internally consistent and mixing them across discards the
reason either works. **Migration:** existing `sys.config.yaml` files are untouched and keep building
what they say. A new configuration written by 0.4 builds a different Hamiltonian from one written by
0.3.x, so do not compare results across the two without checking `inputs/forcefield.json`.

**Default dodecahedral padding is 1.5 nm**, matching production protein–ligand free-energy practice.
2.0 nm remains selectable as `solvent.padding_nm` for unfolded solutes, enhanced sampling that
expands the solute, and highly charged solutes. The built-system cutoff/minimum-image gate is
unchanged and is what is actually enforced.

**`common.barostat_frequency_steps` is a public setting, default 25** — OpenMM's own default,
0.05 ps at 2 fs and 0.10 ps at the optional 4 fs. It reaches every NPT stage, the cMD production
barostat and every REST2 replica, and is recorded in `stage.yaml`, `resolved_stage.yaml`,
`resolved_run.yaml` and `MD/provenance.yaml` in both steps and picoseconds. The constant that used
to live in the generated `md_stages.py` is gone: a generated project now refuses to construct a
barostat at an interval no record names. **Migration:** regenerate `MD/` with `md-gen`; a project
generated by an earlier version has no `barostat_frequency_steps` key and its scripts will say so.

**The implicit ligand route is labelled experimental, with the reason measured.** `sys-gen` reads
the built `CustomGBForce` back and records which atoms carry parameters from the GB-Neck2 fit and
which carry ParmEd's generic fallback, whether mbondi3's residue-specific adjustments could apply at
all, and a `support_status`. Amber `igb=8` parity is claimed only for the peptide/protein route with
full coverage. The peptide route is unchanged.

**4 fs is refused unless hydrogens are constrained and water is rigid**, as well as repartitioned.
HMR is now verified group by group against the same System built without it, and the result is
recorded as `constraints.hmr_group_conservation`.

**Records now carry the full nonbonded treatment and the whole box geometry.** `forcefield.json`
gained the switching-function state and switch distance, the dispersion-correction state, the Ewald
error tolerance, the four box distances under names that say which is which, the salt accounting and
the expanded `<Include>` list of any force-field wrapper; `MD/provenance.yaml` gained a `protocol`
block with the thermostat, timestep, constraints and pressure coupling. A previously silent bug is
fixed: the nonbonded method was written as `"unknown"` because the builder's report was read from
the wrong level of its own record.

**New: `docs/md-defaults-scientific-rationale.md` and its PDF**, with every default's evidence
classified as jointly parameterized, combination benchmarked, component evidence, extrapolation or
implementation documentation, plus a section on what the citations do **not** establish.
References with verified DOIs are in `docs/md-defaults-references.bib`.

**0.3.x data is not retrofitted.** `scripts/retrofit_fair_v030.py` still reads only, preserves a
recorded ff19SB/OPC identity exactly, and leaves a missing value `unknown`.
